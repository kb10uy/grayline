...
enum Request {
    LookUp { callsign: String, refresh: bool },
}
...
impl ContactWorker {
    ...
    pub fn spawn(settings: &ContactSettings, store: Option<PathBuf>, waker: Waker) -> Self {
        let snapshot = Arc::new(Mutex::new(ContactSnapshot::default()));
        let (requests, incoming) = mpsc::channel();
        let worker_snapshot = Arc::clone(&snapshot);
        let opened = open(settings, store);
...
fn open(settings: &ContactSettings, store: Option<PathBuf>) -> Result<Directory, AppError> {
    let store = match store {
        Some(path) => Store::open(&path)?,
        None => Store::in_memory()?,
    };
    Ok(Directory::new(store, remote(settings)))
}
...
fn remote(settings: &ContactSettings) -> Option<Wavelog> {
    if !settings.lookup {
        return None;
    }
    let credentials = grayline_qso::default_credentials_path()
        .map(|path| Credentials::read(&path).unwrap_or_default())
        .unwrap_or_default()
        .with_environment();
...
fn contact_loop(
    ...
    while let Ok(request) = incoming.recv() {
        ...
        for request in pending.drain(..) {
            let Request::LookUp { callsign, refresh } = request;
            look_up(&mut directory, &callsign, refresh, snapshot);
        }
        waker.wake();
    }
