...
ソースコードに付属する `gl-sstv encode` でも、アプリケーションを起動せずに
テンプレートを確認できます(アーカイブには入っていません。使いかたは
リポジトリの `README.md`)。ただし `gl-sstv encode` が定義する変数は
`${station.callsign}` と `${tx.timestamp.*}` だけなので、それ以外の変数を
使うテンプレートはエラーになります。
