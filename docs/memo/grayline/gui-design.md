...
build script embeds `apps/sstv/assets/icon.ico` as an executable resource, and the
...
so `apps/sstv/assets/icon.png` is compiled into the binary and decoded instead, as
...
`grayline-audio` exposes `synthetic_capture`, a capture queue with no device
