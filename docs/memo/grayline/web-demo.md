...
The receive path — `grayline-dsp`, `grayline-sstv-fskid`, `grayline-sstv`, and
`grayline-sstv-rx` — starts no threads, reads no clock, opens no file, and
...
now emits by default, so `apps/web-demo/Cargo.toml` names them in
