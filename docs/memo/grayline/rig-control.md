...
| Layer | Holds | Where |
| --- | --- | --- |
| Transport | The `rigctld` protocol, framed | `grayline-rig` |
...
`grayline-rig`: a scripting host is application policy, and the reusable crate
...
`grayline-rig` speaks the `rigctld` text protocol on a plain TCP socket.
