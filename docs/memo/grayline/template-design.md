...
`assets/templates/` holds the five templates MMSSTV ships as `def1.mtm` through
...
to premultiplied RGBA; `grayline-sstv-template` converts that buffer before returning
...
The `grayline-sstv-template` crate includes a command-line library example that renders
a KDL template at the background image dimensions and saves the composed RGB
image:

```text
cargo run -p grayline-sstv-template --example compose -- template.kdl background.png output.png
```

`grayline-sstv-template/examples/sample.kdl` is a variable-free sample suitable for a
quick render.
...
The `gl-sstv encode` integration instead renders at the selected SSTV mode's
transport dimensions:

```text
cargo run -p grayline-sstv-cli -- encode --callsign N0CALL template.kdl background.png robot36 output.wav
```
